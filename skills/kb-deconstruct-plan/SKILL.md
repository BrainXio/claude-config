...
1. Resolve the master plan path from the argument:
   ...
   - If the argument is a relative slug like `master-plans/cpr-self-healing.md`,
     resolve from `~/.your-org/docs/` first, then fall back to
     `~/dev/.meta/`
   - If `--from-mcp` is passed, use the `your-agent-docs` MCP server
...
1. Plan — write the output to `~/.your-org/planning/<repo>/<slug>-planning.md` using the
...
- [R.D.P. Ritual](~/.your-org/docs/rules-and-regulations/rdp-ritual.md) —
  canonical ritual definition
- [schedule-tasks](../schedule-tasks/SKILL.md) — next step in the pipeline
- [A.E.D. Ritual](~/.your-org/docs/rules-and-regulations/aed-ritual.md) —
