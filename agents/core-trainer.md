...
2. Validate that required hardware/resources are available (tier ≥ 4)
3. Prepare the dataset: tokenization, splitting, formatting for the target model
4. Configure training: learning rate, batch size, epochs, LoRA/QLoRA settings if applicable
5. Execute training with progress tracking and checkpointing
6. Run evaluation on held-out test data
7. Report metrics and write model card to SUMMARY.md
