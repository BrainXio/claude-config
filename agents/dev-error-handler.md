...
2. **Locate affected files** — Use `Glob` to find source files, `Read` to inspect contents
3. **Diagnose root cause** — Identify whether error is syntax, type, import, or logic related
4. **Apply fix** — Use `Edit` or `Write` to correct the issue in place
5. **Verify fix** — Re-run checks to confirm resolution (ruff, mypy, mdformat)
