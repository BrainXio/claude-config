...
## weight: 4 category: essential description: All commit messages must follow conventional commit format; enforced by CI
...
- CI runs commitlint on every push and pull request
- Any non-conventional commit message fails the CI gate
- Force-push of new root history is a known edge case — commitlint may need bypass in that specific scenario
- Amend commits to fix messages; never skip CI with `--no-verify`

## Anti-patterns

- Never use `--no-verify` to bypass the commit message check
- Never use "Initial commit" or "WIP" as commit messages — use `chore: initial commit`
- Never include AI attribution in commit messages (see `00-no-attribution.md`)
